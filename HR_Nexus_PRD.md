# Product Requirements Document (PRD) — HR Nexus (2-hour Hackathon Demo MVP)
**Version:** 1.0  
**Date:** 2026-05-30  
**Owner:** Team HR Nexus  
...
## 1) Summary
**HR Nexus** is a chat-based HR workflow assistant that turns simple text commands into HR actions. For the hackathon, we will deliver a **working MVP demo** in ~2 hours focusing on:

1) **Leave workflow** (request → approve/reject → status)  
2) **Payroll demo via Excel** (upload payroll spreadsheet → generate bank batch file → send/simulate employee notification emails → export results spreadsheet)
...
- Enable payroll processing as a demo flow driven by chat:
  ...
  - On confirm, system generates:
    ...
    - Email notifications sent via provider if configured, otherwise simulated/logged
- Demonstrate automation readiness: clear “API-first, automation adapter” boundaries (even if RPA/bank integration is stubbed).

### Non-Goals (explicitly out of scope for the 2-hour MVP)
...
- Full authentication/SSO and enterprise security hardening (we’ll use a simple demo identity)
...
- Log action in `audit_logs` (optional in MVP, recommended if time)
...
3) On confirm:
   ...
   - Send or simulate emails:
     ...
     - Else: log “email would be sent” in server logs and mark as `SENT_SIMULATED`
...
- **File storage:** Supabase Storage (payroll input + generated artifacts)
- **Email:** optional provider integration (Resend/SendGrid) else simulate

### 9.2 Core endpoints (indicative)
- `POST /api/chat` — parse intent + perform action
- `GET /api/leave-requests` — list leave requests (admin)
- `POST /api/approve` — approve/reject leave request
- `POST /api/payroll/upload` — upload + parse + preview
- `POST /api/payroll/confirm` — generate artifacts + send/simulate emails
- `GET /api/payroll/download` — download artifacts

### 9.3 Data model (minimum tables)
**Leave**
- `employees(id, name, role, manager_id)`
- `leave_requests(id, employee_id, start_date, end_date, type, reason, status, created_at)`

**Payroll**
- `payroll_runs(id, created_at, status, total_amount, input_file_path)`
- `payroll_items(id, run_id, employee_name, email, bank_account, amount, status, message)`

**Optional**
- `audit_logs(id, action, payload, created_at)`

---

## 10) Non-Functional Requirements (pragmatic for demo)
- **Reliability:** predictable parsing (keyword/regex) and explicit confirmation steps
- **Security (demo minimum):**
  - Do not expose bank account numbers in UI beyond last 4 digits (recommended)
  - Do not store secrets in code; use environment variables
- **Auditability (recommended):** log approvals and payroll execution steps
...
- **Ambiguous chat messages** → Use structured command patterns + confirmation
- **Excel parsing issues** → Strict headers; show row-level validation errors
- **Email sending not configured** → Fall back to simulated emails (still impressive)
- **Bank integration complexity** → Use bank batch CSV as demo artifact

---

## 13) Demo Script (Recommended)
1) Chat: “Request leave 2026-06-10 to 2026-06-12 annual reason family”
2) Confirm → show request card with id + PENDING
3) Admin: approve request id
4) Chat: “Status of my leave request {id}” → show APPROVED
5) Chat: “Run payroll”
6) Upload `payroll.xlsx` → preview totals
7) Confirm → download bank batch + payroll result; show “emails sent/simulated”

---

## 14) Implementation Plan (2-hour realistic)
1) Build leave workflow first (chat + admin + DB).
2) Add payroll intent + upload UI + parsing/validation.
3) Generate artifacts (CSV + XLSX).
4) Add email integration if time; otherwise simulate.
5) Polish demo responses + add guardrails/confirmations.
