...
export default function Home() {
  ...
  return (
    <div className="relative flex h-[calc(100dvh-73px)] min-h-0 flex-1 overflow-hidden bg-mist text-ink">
        ...
        <aside className="hidden h-full min-h-0 w-80 shrink-0 overflow-y-auto lg:block">
          <div className="card-floating p-6">
            ...
            <IdentityDropdown
              ...
              onChange={(val) => setActorEmployeeId(val)}
              options={
                employees.length === 0
                  ? [
                      { value: "emp_001", label: "Ava Nguyen (Employee)" },
                      { value: "mgr_001", label: "Minh Tran (Boss)" },
                      { value: "hr_001", label: "Lan Pham (HR)" },
                    ]
                  : employees.map((e) => ({
                      value: e.id,
                      label: `${e.name} (${roleLabel(e.role, e.id)})`,
                    }))
              }
            />
        ...
        <section className="card-floating flex h-full min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
          <div className="flex shrink-0 items-center justify-between border-b border-platinum-tint px-6 py-4">
            ...
            <div className="hidden flex-wrap items-center justify-end gap-2 md:flex">
              <div className="badge-info flex items-center gap-2 !rounded-[36px] !px-3 !py-1.5">
                <div className={`h-2 w-2 rounded-full ${supabaseConnected ? "bg-emerald-400" : "bg-ash"}`} />
                {supabaseConnected ? (supabaseStorage ? "Supabase · Storage" : "Supabase") : "Supabase off"}
              </div>
              <div className="badge-info flex items-center gap-2 !rounded-[36px] !px-3 !py-1.5">
                <div className={`h-2 w-2 rounded-full ${aiEnabled ? "bg-orchid-flash" : "bg-ash"}`} />
            ...
            >
              ...
              <div className="flex items-center gap-3">
                <textarea
                  ...
                  className="w-full flex-1 resize-none rounded-[14px] border border-platinum-tint bg-snow-white px-4 py-3 text-sm text-text-black outline-none focus:border-action-blue focus:ring-2 focus:ring-action-blue/20"
                />
                <button
                  type="submit"
                  disabled={busy}
                  className="btn-primary h-[50px] shrink-0 px-6 disabled:opacity-60 flex items-center justify-center"
                >
                  {busy ? "…" : "Send"}
                </button>
              </div>
  ...
  return (
    <div className="relative mt-4" ref={ref}>
      <button
        ...
        className="flex w-full items-center justify-between rounded-[14px] border border-platinum-tint bg-snow-white px-4 py-3 text-sm text-text-black outline-none transition-all hover:border-action-blue focus:border-action-blue focus:ring-2 focus:ring-action-blue/20 shadow-subtle-2"
      >
        <span className="truncate">{selectedOption?.label}</span>
        <ChevronDown className={`h-4 w-4 shrink-0 text-steel-gray transition-transform ${open ? "rotate-180" : ""}`} />
      </button>

      {open && (
        <div className="absolute z-50 mt-2 w-full overflow-hidden rounded-[14px] border border-pebble bg-snow shadow-subtle animate-in fade-in zoom-in-95 duration-100">
          <div className="max-h-60 overflow-y-auto p-1">
            {options.map((opt) => (
              <button
                ...
                className={`w-full rounded-[10px] px-3 py-2.5 text-left text-sm transition-colors truncate ${
                  opt.value === value
